...
// spec fun at ./sources/coin.move:20:10+60
function {:inline} $2_coin_address_of(s: $signer): int {
    $1_signer_$address_of(s)
}

// spec fun at ./sources/coin.move:21:10+59
...
// fun coin::create_account [verification] at ./sources/coin.move:12:5+210
procedure {:timeLimit 40} $2_coin_create_account$verify(_$t0: $signer) returns ()
{
    ...
    // bytecode translation starts here
    // assume WellFormed($t0) at ./sources/coin.move:12:5+1
    ...
    // assume forall $rsc: ResourceDomain<coin::Account>(): WellFormed($rsc) at ./sources/coin.move:12:5+1
    ...
    // @0 := save_mem(coin::Account) at ./sources/coin.move:12:5+1
    assume {:print "$at(2,167,168)"} true;
    $2_coin_Account_$memory#0 := $2_coin_Account_$memory;

    // trace_local[s]($t0) at ./sources/coin.move:12:5+1
    assume {:print "$track_local(1,0,0):", $t0} $t0 == $t0;

    // $t3 := signer::address_of($t0) on_abort goto L3 with $t4 at ./sources/coin.move:13:23+22
    assume {:print "$at(2,228,250)"} true;
    ...
    // trace_local[address]($t3) at ./sources/coin.move:13:13+7
    assume {:print "$track_local(1,0,1):", $t3} $t3 == $t3;

    // $t5 := exists<coin::Account>($t3) at ./sources/coin.move:14:18+6
    assume {:print "$at(2,269,275)"} true;
    $t5 := $ResourceExists($2_coin_Account_$memory, $t3);

    // $t6 := !($t5) at ./sources/coin.move:14:17+1
    call $t6 := $Not($t5);

    // if ($t6) goto L0 else goto L1 at ./sources/coin.move:14:9+37
    if ($t6) { goto L0; } else { goto L1; }

    // label L1 at ./sources/coin.move:14:44+1
L1:

    // $t7 := 0 at ./sources/coin.move:14:44+1
    assume {:print "$at(2,295,296)"} true;
    $t7 := 0;
    assume $IsValid'u64'($t7);

    // trace_abort($t7) at ./sources/coin.move:14:9+37
    assume {:print "$at(2,260,297)"} true;
    assume {:print "$track_abort(1,0):", $t7} $t7 == $t7;

    // $t4 := move($t7) at ./sources/coin.move:14:9+37
    $t4 := $t7;

    // goto L3 at ./sources/coin.move:14:9+37
    goto L3;

    // label L0 at ./sources/coin.move:15:26+2
    assume {:print "$at(2,324,326)"} true;
L0:

    // $t8 := 1 at ./sources/coin.move:16:22+1
    assume {:print "$at(2,359,360)"} true;
    $t8 := 1;
    assume $IsValid'u64'($t8);

    // $t9 := pack coin::Account($t8) at ./sources/coin.move:15:30+42
    assume {:print "$at(2,328,370)"} true;
    $t9 := $2_coin_Account($t8);

    // move_to<coin::Account>($t9, $t0) on_abort goto L3 with $t4 at ./sources/coin.move:15:9+7
    ...
    if ($abort_flag) {
        assume {:print "$at(2,307,314)"} true;
        $t4 := $abort_code;
        assume {:print "$track_abort(1,0):", $t4} $t4 == $t4;
        goto L3;
    }

    // label L2 at ./sources/coin.move:18:5+1
    assume {:print "$at(2,376,377)"} true;
L2:

    // assert Not(exists[@0]<coin::Account>($t2)) at ./sources/coin.move:25:9+35
    assume {:print "$at(2,590,625)"} true;
    assert {:msg "assert_failed(2,590,625): function does not abort under this condition"}
      !$ResourceExists($2_coin_Account_$memory#0, $t2);
    ...
    // label L3 at ./sources/coin.move:18:5+1
    assume {:print "$at(2,376,377)"} true;
L3:

    // assert exists[@0]<coin::Account>($t2) at ./sources/coin.move:23:5+189
